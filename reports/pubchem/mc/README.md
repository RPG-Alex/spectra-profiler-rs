# `Mc` profile for `pubchem`

This report summarizes how often the target element `Mc` appears across metadata groups in `pubchem`.

## How to interpret this report

This report treats each molecule as **positive** when its molecular formula contains the target element `Mc`. A molecule is **negative** when its formula does not contain `Mc`.
...
This section shows how many formula-bearing molecules have exactly `k` atoms of `Mc`.
The `0` row represents formulas that do not contain `Mc`.

[CSV table](tables/target_atom_count_distribution.csv)

<img src="figures/target_atom_count_distribution.svg" alt="Mc atom-count distribution" />

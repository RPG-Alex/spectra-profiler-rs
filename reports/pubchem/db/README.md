# `Db` profile for `pubchem`

This report summarizes how often the target element `Db` appears across metadata groups in `pubchem`.

## How to interpret this report

This report treats each molecule as **positive** when its molecular formula contains the target element `Db`. A molecule is **negative** when its formula does not contain `Db`.
...
This section shows how many formula-bearing molecules have exactly `k` atoms of `Db`.
The `0` row represents formulas that do not contain `Db`.

[CSV table](tables/target_atom_count_distribution.csv)

<img src="figures/target_atom_count_distribution.svg" alt="Db atom-count distribution" />

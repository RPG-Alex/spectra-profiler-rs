# `Si` profile for `annotated_ms2`

This report summarizes how often the target element `Si` appears across metadata groups in `annotated_ms2`.

## How to interpret this report

This report treats each spectrum as **positive** when its molecular formula contains the target element `Si`. A spectrum is **negative** when its formula does not contain `Si`.
...
This section shows how many formula-bearing spectra have exactly `k` atoms of `Si`.
